...
export const listUsers = asyncHandler(async (req, res) => {
  ...
  if (role && !['super_admin', 'artist_manager', 'artist'].includes(role)) {
    throw new AppError('Invalid role filter. Must be one of: super_admin, artist_manager, artist', 400);
  }
...
export const createUser = asyncHandler(async (req, res) => {
  ...
  if (!validRoles.includes(role)) {
    throw new AppError('Invalid role. Must be one of: super_admin, artist_manager, artist', 400);
  }
...
export const deleteUser = asyncHandler(async (req, res) => {
  ...
  // Check if user exists
  const users = await query('SELECT id, email, role FROM users WHERE id = ?', [id]);
